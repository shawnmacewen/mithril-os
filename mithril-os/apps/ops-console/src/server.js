...
const DEFAULT_DELEGATION_NOTIFY_CHANNEL_ID = "1474209566437671096";
...
const defaultRoutingConfig = {
  ...
  delegationNotifyRouting: {
    defaultChannelId: DEFAULT_DELEGATION_NOTIFY_CHANNEL_ID,
    byTargetAgent: {
      oddeye: "1473821773710491791",
      koda: "",
    },
    bySourceTarget: {},
  },
...
async function resolveDelegationNotifyChannelId({ explicitNotifyChannelId, sourceAgent, targetAgent }) {
  const explicit = String(explicitNotifyChannelId || "").trim();
  if (explicit) return explicit;

  const routing = await readRoutingConfig();
  const notifyCfg = routing.parsed?.delegationNotifyRouting || {};

  const src = normalizeAgentId(sourceAgent);
  const tgt = normalizeAgentId(targetAgent);
  const pairKey = src && tgt ? `${src}->${tgt}` : "";

  const pairMap = notifyCfg.bySourceTarget && typeof notifyCfg.bySourceTarget === "object" ? notifyCfg.bySourceTarget : {};
  const targetMap = notifyCfg.byTargetAgent && typeof notifyCfg.byTargetAgent === "object" ? notifyCfg.byTargetAgent : {};

  const pairChannel = pairKey ? String(pairMap[pairKey] || "").trim() : "";
  if (pairChannel) return pairChannel;

  const targetChannel = tgt ? String(targetMap[tgt] || "").trim() : "";
  if (targetChannel) return targetChannel;

  return String(notifyCfg.defaultChannelId || DEFAULT_DELEGATION_NOTIFY_CHANNEL_ID).trim();
}
...
app.post("/api/agent-routing", async (req, res) => {
  ...
  const merged = {
    ...
    delegationNotifyRouting: {
      ...defaultRoutingConfig.delegationNotifyRouting,
      ...(incoming.delegationNotifyRouting || {}),
      byTargetAgent: {
        ...defaultRoutingConfig.delegationNotifyRouting.byTargetAgent,
        ...(incoming.delegationNotifyRouting?.byTargetAgent || {}),
      },
      bySourceTarget: {
        ...defaultRoutingConfig.delegationNotifyRouting.bySourceTarget,
        ...(incoming.delegationNotifyRouting?.bySourceTarget || {}),
      },
    },
  };
...
app.post("/api/delegations/handoff/start", async (req, res) => {
  ...
  const notifyChannelId = await resolveDelegationNotifyChannelId({
    explicitNotifyChannelId: req.body?.notifyChannelId,
    sourceAgent,
    targetAgent,
  });
  let notifyResult = null;
  if (notify && notifyChannelId) {
    const startedHumanUtc = new Date(started.at).toLocaleString("en-US", {
      dateStyle: "medium",
      timeStyle: "short",
      timeZone: "UTC",
    });
    const msg = [
      `<#${notifyChannelId}> - Starting job for ${targetAgent}`,
      `Job ID: ${jobId} · ${startedHumanUtc} UTC`,
      `Task: ${summary}`,
    ].join("\n");
